...
type requestPayload struct {
	...
	UsePersonCharacter    *dto.BoolValue `json:"use_person_character,omitempty"`
}
...
func (a *TaskAdaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (taskID string, taskData []byte, taskErr *dto.TaskError) {
	...
	common.SetContextKey(c, constant.ContextKeyUpstreamTaskID, dResp.ID)

	responseTaskID := info.PublicTaskID
	if strings.Contains(strings.ToLower(info.OriginModelName), "seedance") {
		...
		responseTaskID = dResp.ID
	}
	ov := dto.NewOpenAIVideo()
	ov.ID = responseTaskID
	ov.TaskID = responseTaskID
