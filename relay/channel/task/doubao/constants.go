...
// videoPriceTable 各模型在不同 (输出分辨率档, 是否含视频输入) 下的上游单价。
// Doubao 使用元/百万 token，Dreamina 使用美元/百万 token；这里只计算同模型价格比值，单位会相消。
...
var videoPriceTable = map[string]map[videoPriceKey]float64{
	...
	"dreamina-seedance-2-5-260628": {
		{hasVideo: false}:                10.7,
		{hasVideo: true}:                 6.4,
		{is1080p: true, hasVideo: false}: 11.7,
		{is1080p: true, hasVideo: true}:  7.0,
	},
}
